...
def load_cached_human_reviews(cache_condition: str,
    ...
    Human proposal text is shared across conditions, so successful novelty
    reviews can be copied for the same title/cohort/evaluator.
    """
    cache_dir = Path(f'data/reviews/ai_reviews/{cache_condition}/novelty')
    ...
    for path in sorted(cache_dir.glob('novelty_reviews_*.json'), reverse=True):
        try:
            with open(path) as f:
                payload = json.load(f)
        ...
        if loaded_from_file:
            logger.info(f"Loaded {loaded_from_file} cached human novelty reviews from {path}")

    logger.info(f"Reusable human novelty reviews available: {len(cache)}")
    return cache
...
def main():
    ...
    human_proposal_dir = Path('data/human-proposals/rephrased')
