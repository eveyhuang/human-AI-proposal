...
- Finding 2 (filtering): Given the same proposal, AI review panels are tighter and less varied than the human panel for nearly every proposal (Cliff's δ down to −1.0 under structured prompting), yet each AI panel individually covers 95 to 98 percent of what the human reviewers said. Each AI review is thorough, but a panel of them says nearly the same thing repeatedly. A human panel, each reviewer says different things and catches things the others missed. [current abstract wording, in use]
...
### §C — AI review panels lose independence, not coverage
...
#### Para C2 — coverage runs the other way
**TS:** Each AI review is individually close to everything the human reviewers said, so the panel loses independence rather than breadth.
...
- Filtering loses little in size but almost all independence, proposal after proposal.
...
- AI reviewers agree with one another, while human reviewers disagree, and that disagreement is the reason a proposal gets more than one reader. The score-level analysis (SI-3) shows this disagreement is measurably absent from the numbers a committee consumes.
- A fully automated pipeline would draw from a poorer menu and judge it with one palate.
